    ...
    <div className="App">
      <header className="App-header">
        ...
        <svg viewBox="0 0 250 250" preserveAspectRatio="xMinYMin meet">
          <path d="M0,200 C75,150 175,250 250,200 L250,250 L0,250 Z" style={{ stroke: 'none', fill: 'white' }}></path>
        </svg>
      </header>
...
function renderUsers(userArray) {
    ...
    for (let i = 0; i < userArray.length; i++) {
        ...
        if (user.savedEvents.length > 0) {
            events = '  |   Attending: ' + user.savedEvents.map(x => x.title).join(', ');
        }
...
$(document).ready(function() {
    ...
    $("#category-search input[type='submit']").click(function(e) {
     ...
     $("#save-user-event input[type='submit']").click(function(e) {
        ...
        fetch("http://localhost:8888/save-user-event", post({ username, title }))
        .then(function(response) {
            response.json().then(function(response){
                renderUsers(response);
            })
